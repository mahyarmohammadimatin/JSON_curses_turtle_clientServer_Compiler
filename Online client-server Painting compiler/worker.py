...
def runfunc(name,args):
    ...
    global t,w,h
    if name=="if":
        if argsmaker(args[0])!=0: return argsmaker(args[1])
        else:
    ...
    if name=="drawLine":
        ...
        def ellipse(t, x, y, r1, r2,r,g,b):
            ...
            t.penup()
            return 0
        return ellipse(t,argsmaker(args[0]),argsmaker(args[1]),argsmaker(args[2]),argsmaker(args[3]),argsmaker(args[4]),argsmaker(args[5]),argsmaker(args[6]))
...
t.getscreen().getcanvas().postscript(width=w,height=h,file="img.ps")
